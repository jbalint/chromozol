...
function broadcastEventToSparqlInsert(msg) {
    ...
    var insertOp = `insert data { graph cmzl: { ${triples} } }`;
    var handleOldLastActivatedTab = "";
    if (msg.event == "tab-activated") {
        // handleOldLastActivatedTab = "; delete { graph cmzl: { ?last a ?Type } } " +
        //     "where { BIND(cmzl:lastActivatedTab as ?Type) ?last a ?Type } ; " +
        //     `insert data { graph cmzl: { cmzl:Tab-${msg.tabId} a cmzl:lastActivatedTab } }`;
        handleOldLastActivatedTab = "; delete { graph cmzl: { ?last a cmzl:LastActivatedTab } } " +
            "where { ?last a cmzl:LastActivatedTab } ; " +
            `insert data { graph cmzl: { cmzl:Tab-${msg.tabId} a cmzl:LastActivatedTab } }`;
    }
    var seq = insertOp + handleOldLastActivatedTab;
    submitSparqlUpdateSequence(seq);
    if (TRACE_MESSAGES) {
...
/**
 * Submit a SPARQL update sequence
 */
function submitSparqlUpdateSequence(updateSeq) {
    var xhr = new XMLHttpRequest();
    //alert(escape(updateSeq));
    ...
    if (true) {
        ...
        xhr.send("query=" + encodeURIComponent(updateSeq));
    } else {
        // this doesn't work...
        var formData = new FormData();
        formData.append("query", updateSeq);
        xhr.send(formData);
    }
    function process(e) {
        if (xhr.readyState === 4) {
            if (xhr.status !== 200) {
                ...
                throw Error("During insert: " + updateSeq);
            }
        }
    }
