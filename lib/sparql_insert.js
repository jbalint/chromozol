...
function toSparqlSessionStart(msg) {
    ...
    triples += `cmzl:Session-${sessionId} a cmzl:Session ; cmzl:hostname "tagore" ; cmzl:sessionId "${msg.sessionId}" . \n`;
    return triples;
}
...
function broadcastEventToSparqlInsert(msg) {
    ...
    var query = `insert data { ${triples} }`;
    var xhr = new XMLHttpRequest();
    //alert(escape(query));
    xhr.open("POST", "https://localhost/stardog/test/update", "true");
